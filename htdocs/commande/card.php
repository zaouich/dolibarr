...
if (empty($reshook)) {
	 ...
	 */ elseif ($action == 'updateline' && $usercancreate && GETPOST('save')) {
		...
		if (!$error) {
			...
			if ($result >= 0) {
				require_once DOL_DOCUMENT_ROOT . '\custom\mail\lib\mail.lib.php';
				send_email_after_create_commande($object, $conf, $langs, $mysoc);
				// get your paiment infos 
				// get commande ref
				$ref = $object->ref;
				// get the commande products 
				$products = $object->lines;
				$products_table = "<table border='1' style='border-collapse: collapse;'><tr><th>Product</th><th>Quantity</th><th>Price</th></tr>";
				foreach ($products as $product) {
					$products_table .= "<tr><td>" . $product->product_label . "</td><td>" . $product->qty . "</td><td>" . $product->total_ht . "</td></tr>";
				}
